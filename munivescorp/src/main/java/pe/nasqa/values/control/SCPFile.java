...
public class SCPFile {
    ...
    public boolean crearCarpetaDia(String directorio,String carpetaFecha) {
        ...
        try {
            ...
            session = jsch.getSession(SFTPUSER, SFTPHOSTE);
    ...
    public List<String> obtenerListadoArchivos(String rutaCarpeta, String directorio) {
        ...
        try {
            ...
            session = jsch.getSession(SFTPUSER, SFTPHOSTE, SFTPPORT);
//            session.setPassword(SFTPPASS);
            java.util.Properties config = new java.util.Properties();
            config.put("StrictHostKeyChecking", "no");
            config.put("UserKnownHostsFile", "/dev/null");
  ...
  //--------------CP 27/11 -------
    ...
    public String uploadFileToSFTP3(String archivo, String rutaDestino, HttpServletRequest request) {
        /*
        String arrayArchivo[] = archivo.split("/");
        String nombreArchivo = new Date().getTime()+" "+arrayArchivo[arrayArchivo.length - 1];
        */
    String sw ="1"; 
        try {
            JSch jsch = new JSch();
            jsch.addIdentity(request.getServletContext().getRealPath("/WEB-INF/jasper/values_ppk.ppk"), SFTPPASS);
            session = jsch.getSession(SFTPUSER, SFTPHOSTE, SFTPPORT);
            //session = jsch.getSession(SFTPUSER, SFTPHOSTE, SFTPPORT);
            session.setPassword(SFTPPASS);
            java.util.Properties config = new java.util.Properties();
