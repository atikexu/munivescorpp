...
public class DistribucionDaoImp implements DistribucionDao{
	...
	*/
	...
	public List<ReporteCoordinacionDia> getReporteCoordinacionDia(String fec_inicio, String fec_fin) {
		// TODO Auto-generated method stub
		String sql =  " select * from frepcoorddia(to_date(:fec_inicio,'DD/MM/YYYY'),to_date(:fec_fin,'DD/MM/YYYY'))";
		Query query = session.getCurrentSession().createSQLQuery(sql).addEntity(ReporteCoordinacionDia.class);		
		query.setString("fec_inicio", fec_inicio);		
		query.setString("fec_fin", fec_fin);	
		System.out.println(query.getQueryString());
		return query.list();
	...
	public List<Object> coordinacionesXCodBarFecha(String codBar,Date fecha){
		String sql =  "select fec_coo, cod_bar, cod_age "
				+ " from distribucion_coord"
				+ " where cod_bar=:codBar"
				+ " and fec_coo>=:fecha";
		Query query = session.getCurrentSession().createSQLQuery(sql);
		query.setString("codBar", codBar);
	...
	public List<Object> listaCoordinacionesFecha(Date fecha){
		String sql =  "select aux,"+
					" case when aux=99 or aux=9999 then '' else razonsocial end as razonsocial2,"+
				" des_pro_val,entregada,total,(total - entregada) as diferencia from ("+
				" select 9999 as aux,'x-------x' as razonsocial,'Total' as des_pro_val, sum(entrega) as entregada,count(*) as total from"+ 
				" ("+
				" select cod_cli,razonsocial,tipo_entrega,des_pro_val,"+ 
				" case when d.ind_sit='2'  then 1 else 0 end as entrega"+
				" from distribucion_coord c "+
				" left join distribucion d"+
				" on c.cod_bar=d.cod_bar"+
				" left join cliente cl"+
				" on d.cod_cli=cl.cod_cliente"+
				" where fec_coo=:fecha "+
				" ) b"+
				" union"+
				" select 99 as aux,razonsocial,'Total' as des_pro_val, sum(entrega) as entregada,count(*) as total from"+ 
				" ("+
				" select cod_cli,razonsocial,tipo_entrega,des_pro_val,"+ 
				" case when d.ind_sit='2'  then 1 else 0 end as entrega"+
				" from distribucion_coord c "+
				...
				" union"+
				" select aux,razonsocial,descripcion, sum(entrega) as entregada,count(*) as total from"+ 
				" ("+
				" select rank() OVER (ORDER BY cod_cli) as aux,cod_cli,razonsocial,tipo_entrega,des_pro_val,"+ 
				" case when d.ind_sit='2'  then 1 else 0 end as entrega, descripcion"+
				" from distribucion_coord c "+
				" left join distribucion d"+
				" on c.cod_bar=d.cod_bar"+
				" left join cliente cl"+
				" on d.cod_cli=cl.cod_cliente"+
				" left join valorado v"+
				" on v.cod_tipo=d.cod_pro_val and v.cod_cliente=d.cod_cli"+
				" where fec_coo=:fecha"+
				" ) b"+
				" group by aux,razonsocial,descripcion"+
				" ) tod"+
				" order by razonsocial,1";
...
//	public List<Object> getRendicionXCodBar(String codBarRutaHoja,Integer id){
//		String sql =  "select  "
//				+ " from hoja_ruta_detalle d, agencia_cliente a"
//				+ " where cod_bar=:codBar"
//				+ " and direccion=cod_agencia";
//		Query query = session.getCurrentSession().createSQLQuery(sql);
//		query.setString("codBar", codBarRutaHoja);
//		query.setInteger("id", id);
//		System.out.println(query.getQueryString());
//		return query.list();
//	}
